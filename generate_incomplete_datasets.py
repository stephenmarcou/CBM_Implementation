...
def create_random_incomplete_dataset(args, num_attribute_groups_remove=1):
    ...
    new_folder_name = f"class_attr_data_10_incomplete_{largest_digit + 1}"
    new_folder_path = os.path.join(args.data_dir, new_folder_name)
    os.makedirs(new_folder_path, exist_ok=True)
    
    with open(new_folder_path + "/info.txt", "w") as f:
        f.write(f"Removed attribute groups: {remove_attribute_parts}\n")
        f.write(f"Removed attribute indices: {remove_attribute_indices}\n")
