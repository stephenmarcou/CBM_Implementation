...
class ImbalancedDatasetSampler(torch.utils.data.sampler.Sampler):
    ...
    def __iter__(self):
        idx = (self.indices[i] for i in torch.multinomial(
            self.weights, self.num_samples, replacement=True).tolist())
        return idx
...
def find_class_imbalance(full_pkl_file_path, multiple_attr=False, attr_idx=-1):
    ...
    print(f"Loaded data from {full_pkl_file_path}, number of samples: {len(data)}, number of attributes: {n_attr}")
